...
sboxes['prost'] = [0, 4, 8, 15, 1, 5, 14, 9, 2, 7, 10, 12, 11, 13, 6, 3,16,20,24,31,17,21,30,25,18,23,26,28,27,29,22,19]
...
sboxes['G0']   = [0,1,2,13,4,7,15,6,8,11,12,9,3,14,10,5]
sboxes['cipher']       = [2, 6, 10, 12, 7, 3, 30, 29, 0, 4, 8, 14, 1, 5, 25, 26, 27, 28, 19, 31, 13, 11, 21, 15, 16, 23, 24, 20, 17, 22, 9, 18]
...
if __name__ == "__main__":
    ...
    for i in range(1,args.G+1):
        print('ASSERT(BVLE(BVPLUS(2,0bin0@b{:d}_0,0bin0@b{:d}_1,0bin0@b{:d}_2),0bin01));'.format(i,i,i))
        print('ASSERT(BVPLUS(3',end='')
        for j in range(0,n):
            print(',0bin00@a{:d}_{:d}'.format(i,j),end='')
        print(')=0bin001);')
        print('ASSERT(BVPLUS(3,0bin00@a{:d}_5,0bin00@a{:d}_6,0bin00@a{:d}_7,0bin00@a{:d}_8,0bin00@a{:d}_9)=0bin001);'.format(i,i,i,i,i))
        print('ASSERT(BVPLUS(3,0bin00@a{:d}_10,0bin00@a{:d}_11,0bin00@a{:d}_12,0bin00@a{:d}_13,0bin00@a{:d}_14)=0bin001);'.format(i,i,i,i,i))
        print('ASSERT(BVLE(BVPLUS(2,0bin0@a{:d}_0,0bin0@a{:d}_5,0bin0@a{:d}_10),0bin01));'.format(i,i,i))
        print('ASSERT(BVLE(BVPLUS(2,0bin0@a{:d}_1,0bin0@a{:d}_6,0bin0@a{:d}_11),0bin01));'.format(i,i,i))
        print('ASSERT(BVLE(BVPLUS(2,0bin0@a{:d}_2,0bin0@a{:d}_7,0bin0@a{:d}_12),0bin01));'.format(i,i,i))
        print('ASSERT(BVLE(BVPLUS(2,0bin0@a{:d}_3,0bin0@a{:d}_8,0bin0@a{:d}_13),0bin01));'.format(i,i,i))
        print('ASSERT(BVLE(BVPLUS(2,0bin0@a{:d}_4,0bin0@a{:d}_9,0bin0@a{:d}_14),0bin01));'.format(i, i, i))

    for i in range(0, 32):
        for j in range(0, n):
            print(
                'ASSERT(y_{:d}=BVPLUS(1,BVMULT(1,k{:d}_0,x{:d}_{:d}),BVMULT(1,k{:d}_1,x{:d}_{:d}),BVMULT(1,k{:d}_2,x{:d}_{:d}),BVMULT(1,k{:d}_3,x{:d}_{:d}),BVMULT(1,k{:d}_4,x{:d}_{:d})'
                '  ));'.format( i * n + j, j,args.G, i * n, j, args.G,i * n + 1, j,args.G, i * n + 2, j, args.G,i * n + 3,j,args.G,i*n+4))

    for i in range(0, 5):
        print(
            'ASSERT(BVPLUS(3,0bin00@k{:d}_0,0bin00@k{:d}_1,0bin00@k{:d}_2,0bin00@k{:d}_3,0bin00@k{:d}_4)=0bin001);'.format(i, i, i, i,i))
        print(
            'ASSERT(BVPLUS(3,0bin00@k0_{:d},0bin00@k1_{:d},0bin00@k2_{:d},0bin00@k3_{:d},0bin00@k4_{:d})=0bin001);'.format(i, i, i, i,i))
    ...
    for l in range(0,3):
        ...
        print('));')
    print('ASSERT(T=BVPLUS(8,0bin0000@T2,BVMULT(8,0bin0000@T3,0bin00000101)));')
